...
export const AdminProductCreate = () => {
  const { lang, setAdminTab, triggerHaptic, getAdminHeaders, categories, products, fetchProducts } = useStore();
  ...
  const [form, setForm] = useState({
    ...
    category_id: activeCategories.length > 0 ? activeCategories[0].id : '',
  ...
  const handleSubmit = async (e) => {
    ...
    const exists = products.some(p => 
      (p.title_uz || p.title?.uz || (typeof p.title === 'string' ? p.title : '')).trim().toLowerCase() === form.title_uz.trim().toLowerCase()
    );
    if (exists) {
      triggerHaptic('warning');
      alert(lang === 'uz' ? 'Bunday nomli mahsulot allaqachon mavjud!' : 'Товар с таким названием уже существует!');
      return;
    }
